...
def test_get_course_list(dc: DataClient):
    assert isinstance(dc.get_column_list(), dict)


def test_get_course_intro(dc: DataClient):
    assert dc.get_column_intro(212)


def test_get_post_content(dc: DataClient):
    assert dc.get_article_content(333)


def test_get_course_content(dc: DataClient):
    assert dc.get_articles_of_column(212)


def test_local_storage(dc: DataClient):
    course_id = 212
    dc.get_column_intro(course_id)
...
def test_force(dc: DataClient):
    ...
    course = Query()
    # read from gk api
    res = dc.get_column_intro(course_id)
    ...
    res = dc.get_column_intro(course_id, force=True)

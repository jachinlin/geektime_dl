...
class MailServer(object):
    """
    Represents an SMTP server, able to send outgoing emails,
    with SSL and TLS capabilities.
    """
    ...
    def connect(self):
        ...
        if smtp_encryption == 'ssl':
            connection = smtplib.SMTP_SSL(
                smtp_server, smtp_port, timeout=self._smtp_timeout)
        else:
    ...
    def build_email(
            self, email_to, subject, body, attachments=None, subtype='plain'):
        """
        Constructs an RFC2822 email.message.Message object
        based on the keyword arguments passed, and returns it.

       :param list email_to: list of recipient addresses
                            (to be joined with commas)
       :param string subject: email subject (no pre-encoding/quoting necessary)
       :param string body: email body, of the type ``subtype``
                           (by default, plaintext).
                           If html subtype is used, the message will be
                           automatically converted to plaintext and wrapped in
                           multipart/alternative, unless an explicit
                           ``body_alternative`` version is passed.
       :param string subtype: optional mime subtype for the text body
                              (usually 'plain' or 'html'),
                              must match the format of the ``body`` parameter.
                              Default is 'plain',
                              making the content part of the mail "text/plain".
       :param list attachments: list of (filename, filecontents) pairs,
                                where filecontents is a string
       ...
       :return: the new RFC2822 email message
        ...
        email_text_part = MIMEText(
            body or u'', _subtype=subtype, _charset='utf-8')
    ...
    def send_email(self, message):
        ...
        try:
            ...
            try:
                smtp.sendmail(
                    self._smtp_user, message['To'], message.as_string())
            finally:
                smtp.quit()
        except Exception as e:
